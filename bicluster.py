...
class Bicluster:
    ...
    id: str = field(default_factory=lambda: f"{BICLUSTER_ID_PREFIX}{uuid.uuid4().hex[:8]}")
    
    def __post_init__(self):
        """Validate bicluster after initialization."""
        if not isinstance(self.row_indices, np.ndarray) or self.row_indices.dtype != np.bool_:
            raise TypeError("row_indices must be a boolean NumPy array.")
        if not isinstance(self.col_indices, np.ndarray) or self.col_indices.dtype != np.bool_:
        ...
        if np.sum(self.row_indices) == 0 and np.sum(self.col_indices) == 0 and len(self.row_indices) > 0 and len(self.col_indices) > 0:
    ...
    def overlaps_with(self, other: Bicluster, threshold: float = 0.25) -> bool:
        ...
        if self.row_indices.shape != other.row_indices.shape or \
           self.col_indices.shape != other.col_indices.shape:
            ...
            raise ValueError("Cannot compare biclusters from different underlying matrix shapes or with incompatible index dimensions.")
    ...
    def intersection_area(self, other: Bicluster) -> int:
        ...
        if self.row_indices.shape != other.row_indices.shape or \
           self.col_indices.shape != other.col_indices.shape:
            raise ValueError("Cannot compute intersection for biclusters with incompatible index dimensions.")
    ...
    def extract_submatrix(self, matrix: Matrix) -> Matrix:
        ...
        if len(self.row_indices) > matrix.shape[0] or len(self.col_indices) > matrix.shape[1]:
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any], original_row_dim: Optional[int]=None, original_col_dim: Optional[int]=None) -> Bicluster:
        ...
        final_row_dim = original_row_dim if original_row_dim is not None else data.get('original_row_dim')
        final_col_dim = original_col_dim if original_col_dim is not None else data.get('original_col_dim')
    ...
    def __eq__(self, other: object) -> bool:
        ...
        if self.id == other.id and self.id is not None: # if IDs match and are not None, they are equal
            return True
