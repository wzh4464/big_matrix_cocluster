...
# 模拟搜索过程
def simulate_search(
    p: float, N: int, M: int, T_max: int, num_simulations: int, n_jobs: int
) -> List[float]:
    expected_results = []
    for T in tqdm(range(70, T_max + 1), desc="Simulating search process"):
        results = Parallel(n_jobs=n_jobs)(
            delayed(single_search_simulation)(T, p, N, M)
...
plt.plot(
    range(70, T_max + 1), expected_results, label="Expected number of found co-clusters"
)
