...
def bicluster_b() -> Bicluster:
    return Bicluster(
        R_IDX_B.copy(), C_IDX_B.copy(), SCORE_B, META_B.copy()
    )  # Intentionally use META_B to test equality more deeply


@pytest.fixture
def bicluster_c() -> (
    Bicluster
):  # Overlaps with A only on rows, different col intersection behavior
    return Bicluster(
        row_indices=R_IDX_C.copy(), col_indices=C_IDX_C.copy(), score=SCORE_C
    )
...
def test_bicluster_equality(bicluster_a: Bicluster):
    bc_a_copy = Bicluster(
        R_IDX_A.copy(), C_IDX_A.copy(), SCORE_A, META_B.copy(), id=bicluster_a.id
    )
    ...
    bc_a_diff_id = Bicluster(
        R_IDX_A.copy(), C_IDX_A.copy(), SCORE_A, META_B.copy(), id="some_other_id"
    )
    ...
    bc_a_content_diff = Bicluster(
        R_IDX_A.copy(),
        C_IDX_A.copy(),
        score=0.1,
        metadata=META_B.copy(),
        id="another_unique_id",
    )
    bc_a_ref_for_content = Bicluster(
        R_IDX_A.copy(),
        C_IDX_A.copy(),
        score=SCORE_A,
        metadata=META_B.copy(),
...
def test_bicluster_overlaps_and_jaccard(bicluster_a: Bicluster, bicluster_b: Bicluster):
    ...
    assert not bicluster_a.overlaps_with(
        bicluster_b
    )  # Default threshold 0.25; 1/14 is ~0.0714
    ...
    no_overlap_rows = np.array(
        [False, False, False, False, False]
    )  # Ensure different length if needed or handle in Bicluster
...
def test_intersection_ratio(bicluster_a: Bicluster, bicluster_b: Bicluster):
    ...
    assert_allclose(
        bicluster_b.intersection_ratio(bicluster_a), 1 / 6
    )  # Symmetrical if using min area
    ...
    bc_empty = Bicluster(
        np.zeros(len(R_IDX_A), dtype=bool), np.zeros(len(C_IDX_A), dtype=bool), 0.1
    )
