...
from .core import BiclusterConfig, Matrix, ClusteringMethod, ScoringMethod # Add ScoringMethod
from .bicluster import Bicluster, IntArray # IntArray might be needed from .core or defined in .bicluster
...
class BiclusterDetector(ABC):
    ...
    def validate_bicluster(self, matrix: Matrix, bicluster: Bicluster, min_rows: int, min_cols: int, tolerance: float) -> bool:
        ...
        meets_size_criteria = bicluster.shape[0] >= min_rows and bicluster.shape[1] >= min_cols
...
class SVDBiclusterDetector(BiclusterDetector):
    ...
    def detect(self, 
        ...
        cfg_k1 = n_iterations if n_iterations is not None else self.config.k1 # k1 is iterations for SVD refinement (not directly used like this now)
                                                                             # For now, map n_iterations to k1 if provided for row clustering config.
        cfg_k2_rows = n_clusters_rows if n_clusters_rows is not None else self.config.k1 # Default k1 for row clusters
        cfg_k2_cols = n_clusters_cols if n_clusters_cols is not None else self.config.k2 # Default k2 for col clusters
        ...
        cfg_max_overlap = max_overlap if max_overlap is not None else self.config.max_overlap
        cfg_tolerance = tolerance if tolerance is not None else self.config.tolerance

        self.logger.info(f"Starting bicluster detection on {matrix.shape} matrix using {self.config.clustering_method.value} and {self.config.scoring_method.value}")
        
        if matrix.ndim != 2 or matrix.shape[0] < 2 or matrix.shape[1] < 2:
            self.logger.warning("Matrix is too small for SVD-based biclustering. Returning empty list.")
            return []
        ...
        actual_k_rows = min(cfg_k2_rows, matrix.shape[0], U.shape[1] if U.shape[1] > 0 else matrix.shape[0])
        actual_k_cols = min(cfg_k2_cols, matrix.shape[1], Vh.shape[0] if Vh.shape[0] > 0 else matrix.shape[1])

        if actual_k_rows <= 1 or actual_k_cols <= 1:
            self.logger.warning(f"Number of clusters (k1={actual_k_rows}, k2={actual_k_cols}) too small. Min 2 required. Returning empty list.")
            return []
        ...
        if row_features.shape[0] < actual_k_rows or col_features.shape[0] < actual_k_cols:
            self.logger.warning(f"Not enough samples for clustering: rows ({row_features.shape[0]} vs k1={actual_k_rows}), cols ({col_features.shape[0]} vs k2={actual_k_cols}). Adjusting k.")
        ...
        generated_biclusters = self._generate_biclusters(matrix, row_labels, col_labels, actual_k_rows, actual_k_cols, cfg_min_rows, cfg_min_cols, cfg_tolerance)
        
        # Filter by overlap
        if cfg_max_overlap is not None and cfg_max_overlap < 1.0:
            final_biclusters = self._filter_by_overlap(generated_biclusters, cfg_max_overlap)
        else:
            final_biclusters = generated_biclusters

        self.logger.info(f"Detected {len(final_biclusters)} valid biclusters after overlap filtering.")
        return final_biclusters
    
    def _compute_svd(self, matrix: Matrix, n_components_override: Optional[int] = None) -> Tuple[Matrix, NDArray, Matrix]:
        ...
        if cache_key not in self.svd_cache:
            ...
            n_components = n_components_override if n_components_override is not None else min(matrix.shape) - 1
            ...
            S_inv[S_singular_values > 1e-9] = 1.0 / S_singular_values[S_singular_values > 1e-9]
    ...
    def _cluster_features(self, features: Matrix, n_clusters: int) -> IntArray:
        ...
        if n_clusters <= 1: # KMeans requires n_clusters >= 2 usually.
            self.logger.warning(f"n_clusters is {n_clusters}, which is too small for clustering. Returning all zeros.")
            return np.zeros(features.shape[0], dtype=int) 

        if features.shape[0] < n_clusters:
            self.logger.warning(f"Number of samples ({features.shape[0]}) is less than n_clusters ({n_clusters}). Adjusting n_clusters.")
        ...
        if self.config.clustering_method == ClusteringMethod.KMEANS:
            ...
            raise NotImplementedError(f"Clustering method {self.config.clustering_method.value} not implemented in SVDBiclusterDetector")
    
    def _generate_biclusters(self, matrix: Matrix, row_labels: IntArray, col_labels: IntArray, 
                             num_row_clusters: int, num_col_clusters: int, 
                             min_rows: int, min_cols: int, tolerance: float) -> List[Bicluster]:
        ...
        for i in range(num_row_clusters): # Iterate up to actual_k1 used
            for j in range(num_col_clusters): # Iterate up to actual_k2 used
                ...
                candidate = Bicluster(
                    ...
                    score=float('nan'),  # Placeholder, will be set by validate_bicluster
                    metadata={'cluster_id': (i, j), 'detection_method': 'SVD'}
                )
                
                # Validate bicluster (this will also calculate and set its score)
                if self.validate_bicluster(matrix, candidate, min_rows, min_cols, tolerance):
                    biclusters.append(candidate)
        
        return biclusters 

    def _filter_by_overlap(self, biclusters: List[Bicluster], max_overlap: float) -> List[Bicluster]:
        if not biclusters or max_overlap >= 1.0:
            return biclusters
        # Sort by score (lower is better)
        biclusters.sort(key=lambda bc: bc.score if bc.score is not None else float('inf'))
        ...
        for candidate_bc in biclusters:
            ...
            for selected_bc in selected_biclusters:
                ...
                row_intersect = np.sum(candidate_bc.row_indices & selected_bc.row_indices)
                row_union = np.sum(candidate_bc.row_indices | selected_bc.row_indices)
                col_intersect = np.sum(candidate_bc.col_indices & selected_bc.col_indices)
                ...
                if max(j_row, j_col) > max_overlap: # If either row or col Jaccard exceeds, it's too overlapping
