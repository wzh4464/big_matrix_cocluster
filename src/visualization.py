...
class SyntheticDataGenerator:
    ...
    def __init__(self, config: SyntheticDataConfig):
        ...
        self.ground_truth_biclusters_info: List[Dict[str, Any]] = (
            []
        )  # Info about true biclusters
    ...
    def generate(self) -> Tuple[NDArray, NDArray, List[Bicluster]]:
        ...
        """
        self.logger.info(
            f"Generating {self.config.matrix_shape} matrix with {len(self.config.bicluster_specs)} biclusters"
        )

        # Create base matrix and ground truth Bicluster objects
        structured_matrix, ground_truth_biclusters_list = (
            self._create_structured_matrix_and_gt()
        )
        ...
        permuted_ground_truth_biclusters = self._permute_ground_truth(
            ground_truth_biclusters_list
        )
    ...
    def _create_structured_matrix_and_gt(self) -> Tuple[NDArray, List[Bicluster]]:
        ...
        matrix = self._rng.normal(
            loc=0, scale=self.config.background_noise, size=(rows, cols)
        )  # Background noise

        # Place biclusters sequentially (can be made more complex, e.g., random placement, overlap)
        current_row, current_col = 0, 0
        self.ground_truth_biclusters_info = (
            []
        )  # Store dict info for easy reference/serialization
        ...
        for i, spec in enumerate(self.config.bicluster_specs):
            if current_row + spec.rows > rows or current_col + spec.cols > cols:
                self.logger.warning(
                    f"Bicluster spec {i} ({spec.rows}x{spec.cols}) doesn't fit sequentially. Skipping."
                )
                continue
            ...
            if current_row >= rows:  # Move to next column band if out of rows
                ...
                if current_col >= cols:
                    self.logger.info(
                        "Ran out of space for sequential bicluster placement."
                    )
                    break
    ...
    def _permute_ground_truth(self, gt_biclusters: List[Bicluster]) -> List[Bicluster]:
        ...
        if self.row_permutation is None or self.col_permutation is None:
            raise RuntimeError(
                "Permutation has not been applied yet. Call generate() first."
            )
...
class BiclusterVisualizer:
    ...
    def _figure_context(
        self,
        num_subplots_rows: int = 1,
        num_subplots_cols: int = 1,
        title: Optional[str] = None,
        custom_figsize: Optional[Tuple[float, float]] = None,
    ):
    ...
    def plot_matrix_comparison(
        ...
        with self._figure_context(
            1, 2, title=title, custom_figsize=(self.figsize[0] * 1.5, self.figsize[1])
        ) as (fig, axs):
            ...
            if len(biclusters) > 0:
                ...
                ax2.imshow(
                    np.zeros_like(original), cmap="gray", aspect="auto"
                )  # Show blank
    ...
    def plot_bicluster_statistics(
        ...
        scores = [
            ...
            if bc.score is not None and not np.isnan(bc.score)
        ]
        ...
        with self._figure_context(
            ...
            plot_cols,
            title=title,
            custom_figsize=(self.figsize[0] * 1.2, self.figsize[1] * 1.8),
        ) as (fig, axs):
            ...
            if scores:
                ...
                ax.hist(
                    scores,
                    bins=min(20, len(scores) // 2 if len(scores) > 20 else 10),
                    alpha=0.7,
                    color="skyblue",
                    edgecolor="black",
                )
            ...
            if sizes:
                ...
                ax.hist(
                    sizes,
                    bins=min(20, len(sizes) // 2 if len(sizes) > 20 else 10),
                    alpha=0.7,
                    color="lightcoral",
                    edgecolor="black",
                )
                ax.set_title("Size Distribution (elements)")
                ax.set_xlabel("Bicluster Size")
                ax.set_ylabel("Frequency")
                plot_idx += 1
            ...
            if row_counts and col_counts:
                ax = axs_flat[plot_idx]
                ax.scatter(
                    col_counts, row_counts, alpha=0.6, color="purple", edgecolor="black"
                )  # Swapped to (cols, rows) for common convention
                ax.set_title("Bicluster Dimensions")
    ...
    def _create_bicluster_overlay_matrix(
        self, matrix_shape: Tuple[int, int], biclusters: List[Bicluster]
    ) -> NDArray:
        ...
        for i, bicluster in enumerate(biclusters, 1):  # Start IDs from 1
            ...
            for r_idx in np.where(rows_mask_for_bc)[0]:
                overlay[r_idx, cols_mask_for_bc] = (
                    i  # Assign bicluster ID to cells in these rows that are also in selected columns
                )
        return overlay

    def plot_individual_biclusters(
        self,
        ...
        with self._figure_context(
            plot_rows,
            plot_cols,
            title=f"Top {num_to_plot} Individual Biclusters",
            custom_figsize=(
                self.figsize[0] * plot_cols * 0.4,
                self.figsize[1] * plot_rows * 0.6,
            ),
        ) as (fig, axs_obj):
            ...
            for i, bc in enumerate(biclusters[:num_to_plot]):
                ...
                try:
                    ...
                    ax.set_title(
                        f"{title_prefix} {i+1} ({bc.shape[0]}x{bc.shape[1]}) Score: {bc.score:.3f}",
                        fontsize=9,
                    )
    ...
    def create_report_visualizations(
        ...
        max_individual_biclusters: int = 6,
    ) -> None:
        ...
        self.plot_bicluster_statistics(
            biclusters, save_path=report_dir / "bicluster_statistics.png"
        )
        if biclusters:
...
def create_synthetic_data_with_generator(
    ...
    bicluster_size_range: Tuple[int, int] = (
        15,
        25,
    ),  # (min_dim, max_dim) for rows/cols
    ...
    rng_spec = np.random.RandomState(
        random_state
    )  # Separate RNG for spec generation for reproducibility
    ...
    for i in range(n_biclusters):
        ...
        specs.append(
            BiclusterSpec(
                rows=rows, cols=cols, value=value, noise_level=noise_level_spec
            )
        )
    ...
    permuted_matrix, original_structured_matrix, ground_truth_biclusters = (
        generator.generate()
    )

    return (
        permuted_matrix,
        original_structured_matrix,
        ground_truth_biclusters,
        generator,
    )
...
if __name__ == "__main__":
    ...
    perm_matrix, orig_matrix, gt_biclusters, s_generator = (
    ...
    if gt_biclusters:
        print(
            f"  Example GT Bicluster 0 shape: {gt_biclusters[0].shape}, metadata: {gt_biclusters[0].metadata}"
        )
    ...
    visualizer.plot_matrix_heatmap(
        orig_matrix, title="Original Structured Matrix (Ground Truth)"
    )

    # Plot permuted matrix (input for algorithm)
    visualizer.plot_matrix_heatmap(
        perm_matrix, title="Permuted Matrix (Algorithm Input)"
    )
    ...
    visualizer.plot_matrix_comparison(
        perm_matrix,
        detected_biclusters_demo,
        title="Permuted Matrix with (Simulated) Detected Biclusters",
    )
    ...
    if detected_biclusters_demo:
        visualizer.plot_bicluster_statistics(
            detected_biclusters_demo,
            title="Statistics of (Simulated) Detected Biclusters",
        )
        visualizer.plot_individual_biclusters(
            perm_matrix, detected_biclusters_demo, max_to_plot=2
        )
