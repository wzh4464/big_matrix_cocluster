...
class CompatibilityScorer(ScoringStrategy):
    ...
    def __init__(
        self,
        method: ScoringMethod = ScoringMethod.EXPONENTIAL,
    ...
    def score(self, matrix: Matrix, bicluster: Bicluster) -> float:
        ...
        if (
            bicluster.metadata and "id" in bicluster.metadata
        ):  # Prefer metadata ID if explicitly set for some reason
        ...
        try:
            ...
            if submatrix.size == 0:
                self.logger.debug(
                    f"Bicluster {bicluster_id_for_log} results in empty submatrix. Score is inf."
                )
                return float("inf")
            if submatrix.shape[0] < 2 or submatrix.shape[1] < 2:
                self.logger.debug(
                    f"Bicluster {bicluster_id_for_log} submatrix too small ({submatrix.shape}). Score is inf."
                )
                return float("inf")
            return self._calculate_compatibility(submatrix)
        except ValueError as ve:
            self.logger.warning(
                f"ValueError for bicluster {bicluster_id_for_log}: {ve}. Returning inf score."
            )
            return float("inf")
        except Exception as e:
            self.logger.error(
                f"Unexpected error for bicluster {bicluster_id_for_log}: {e}",
                exc_info=True,
            )
            return float("inf")
    ...
    def _calculate_compatibility(self, submatrix: Matrix) -> float:
        ...
        noisy_submatrix = submatrix + self.noise_level * np.random.randn(
            *submatrix.shape
        )
        ...
        if self.method == ScoringMethod.PEARSON:
            ...
            self.logger.warning(
                "COMPATIBILITY scoring not fully implemented, using EXPONENTIAL."
            )
            return self._exponential_compatibility(noisy_submatrix)
        else:
    ...
    def _exponential_compatibility(self, submatrix: Matrix) -> float:
        ...
        ):  # Can operate on 1xN or Nx1, though score_helper handles < 2
            return float("inf")
        ...
        if num_rows >= 2:
            for i in range(num_rows):
                for j in range(i + 1, num_rows):
                    ...
                    row_sim_matrix[i, j] = row_sim_matrix[j, i] = np.exp(
                        -dist_sq / num_cols
                    )
        ...
        if num_cols >= 2:
            for i in range(num_cols):
                for j in range(i + 1, num_cols):
                    dist_sq = np.sum((submatrix[:, i] - submatrix[:, j]) ** 2)
                    col_sim_matrix[i, j] = col_sim_matrix[j, i] = np.exp(
                        -dist_sq / num_rows
                    )
        ...
        if (
            num_rows < 2 and num_cols < 2
        ):  # e.g. 1x1 matrix, should be perfect score by this logic
            return 0.0
