...
from big_matrix_cocluster.bicluster import Bicluster
from big_matrix_cocluster.core import Matrix
from big_matrix_cocluster.visualization import (
    BiclusterSpec,
    SyntheticDataConfig,
...
def test_synthetic_data_generator_initialization(generator: SyntheticDataGenerator, simple_generator_config: SyntheticDataConfig):
...
def test_generate_bicluster_data(generator: SyntheticDataGenerator):
    ...
    generator._rng = np.random.RandomState(DEFAULT_RANDOM_STATE) # Reset for predictable noise
    ...
    assert_allclose(np.mean(submatrix), expected_mean, atol=spec.noise_level * 3) # Wider tolerance, mean is sensitive
...
def test_create_structured_matrix_and_gt(simple_generator_config: SyntheticDataConfig):
    ...
    assert_allclose(np.mean(sub_bc1), bc1_spec.value, atol=bc1_spec.noise_level * 5 + config.background_noise * 5)
    assert gt_bc1.shape == (bc1_spec.rows, bc1_spec.cols)
    assert gt_bc1.metadata['value'] == bc1_spec.value
    assert_array_equal(gt_bc1.row_indices, np.array([True]*bc1_spec.rows + [False]*(config.matrix_shape[0]-bc1_spec.rows)))
    assert_array_equal(gt_bc1.col_indices, np.array([True]*bc1_spec.cols + [False]*(config.matrix_shape[1]-bc1_spec.cols)))
    ...
    assert_allclose(np.mean(sub_bc2), bc2_spec.value, atol=bc2_spec.noise_level * 5 + config.background_noise * 5)
    ...
    assert_array_equal(gt_bc2.col_indices, np.array([True]*bc2_spec.cols + [False]*(config.matrix_shape[1]-bc2_spec.cols)))
    assert len(generator_multi.ground_truth_biclusters_info) == len(specs)

def test_create_structured_matrix_gt_bicluster_too_large(simple_generator_config: SyntheticDataConfig):
    # Test scenario where a bicluster spec is too large to fit
    large_spec = [BiclusterSpec(rows=simple_generator_config.matrix_shape[0] + 1, cols=5)]
    ...
    assert_allclose(np.mean(matrix), 0, atol=config_too_large.background_noise * 5 + 0.1) # Wider tolerance for overall matrix mean
...
def test_apply_permutation(generator: SyntheticDataGenerator):
    ...
    if not (np.all(generator.row_permutation == np.arange(original_matrix.shape[0])) and 
            np.all(generator.col_permutation == np.arange(original_matrix.shape[1]))):
...
def test_permute_ground_truth(generator: SyntheticDataGenerator):
    ...
    generator.row_permutation = np.array([2,0,1,3,5,4]) # Original row 0 goes to 2, 1 to 0, 2 to 1 etc.
    generator.col_permutation = np.array([1,0,3,2,4]) # Original col 0 goes to 1, 1 to 0, 2 to 3 etc.
...
def test_generate_full(simple_generator_config: SyntheticDataConfig):
    ...
    assert_allclose(np.sum(p_matrix), np.sum(s_matrix), rtol=1e-5) # Sums might differ slightly due to float precision if any in-place ops happened, but shouldn't here.

    assert generator_full.row_permutation is not None
    assert generator_full.col_permutation is not None
    assert len(generator_full.ground_truth_biclusters_info) == len(simple_generator_config.bicluster_specs)
...
def test_generate_full_no_specs():
    ...
    assert_allclose(np.mean(p_matrix), 0, atol=config_no_specs.background_noise * 5 + 0.1)
...
def sample_biclusters_vis(sample_matrix_vis: Matrix) -> list[Bicluster]:
    ...
    bc1_rows = np.zeros(rows, dtype=bool); bc1_rows[0:5] = True
    bc1_cols = np.zeros(cols, dtype=bool); bc1_cols[0:5] = True
    bc1 = Bicluster(bc1_rows, bc1_cols, score=0.1, metadata={'id': 'vis_bc1'})

    bc2_rows = np.zeros(rows, dtype=bool); bc2_rows[10:15] = True
    bc2_cols = np.zeros(cols, dtype=bool); bc2_cols[8:12] = True
...
def test_visualizer_plot_matrix_heatmap_no_error(mock_show, visualizer: BiclusterVisualizer, sample_matrix_vis: Matrix):
    try:
...
def test_visualizer_plot_matrix_heatmap_save(mock_savefig, mock_show, visualizer: BiclusterVisualizer, sample_matrix_vis: Matrix, tmp_path: Path):
...
@patch('matplotlib.pyplot.show')
def test_visualizer_plot_matrix_comparison_no_error(mock_show, visualizer: BiclusterVisualizer, sample_matrix_vis: Matrix, sample_biclusters_vis: list[Bicluster]):
    try:
        visualizer.plot_matrix_comparison(sample_matrix_vis, sample_biclusters_vis)
        visualizer.plot_matrix_comparison(sample_matrix_vis, []) # Test with no biclusters
    except Exception as e:
        pytest.fail(f"plot_matrix_comparison raised an exception: {e}")
    assert mock_show.call_count == 2

@patch('matplotlib.pyplot.show')
def test_visualizer_plot_bicluster_statistics_no_error(mock_show, visualizer: BiclusterVisualizer, sample_biclusters_vis: list[Bicluster]):
    try:
        visualizer.plot_bicluster_statistics(sample_biclusters_vis)
        visualizer.plot_bicluster_statistics([]) # Test with no biclusters (should warn and return)
    except Exception as e:
...
@patch('matplotlib.pyplot.show')
def test_visualizer_plot_individual_biclusters_no_error(mock_show, visualizer: BiclusterVisualizer, sample_matrix_vis: Matrix, sample_biclusters_vis: list[Bicluster]):
    try:
        visualizer.plot_individual_biclusters(sample_matrix_vis, sample_biclusters_vis, max_to_plot=1)
        visualizer.plot_individual_biclusters(sample_matrix_vis, []) # Test with no biclusters
    except Exception as e:
...
def test_create_bicluster_overlay_matrix(visualizer: BiclusterVisualizer, sample_matrix_vis: Matrix, sample_biclusters_vis: list[Bicluster]):
    overlay = visualizer._create_bicluster_overlay_matrix(sample_matrix_vis.shape, sample_biclusters_vis)
...
def test_visualizer_create_report_visualizations(
    ...
    visualizer.create_report_visualizations(sample_matrix_vis, sample_biclusters_vis, report_dir)

    mock_mkdir.assert_called_once_with(parents=True, exist_ok=True)
    mock_plot_compare.assert_called_once_with(
        sample_matrix_vis, sample_biclusters_vis, 
