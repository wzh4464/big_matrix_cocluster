...
from big_matrix_cocluster.core import (BiclusterConfig, ScoringMethod, 
                                       ClusteringMethod, Matrix)
from big_matrix_cocluster.detection import SVDBiclusterDetector
...
def test_svd_detector_initialization(core_config_detection: BiclusterConfig):
    with patch('big_matrix_cocluster.detection.CompatibilityScorer') as MockScorer:
...
def test_detect_simple_bicluster_kmeans(core_config_detection: BiclusterConfig, simple_matrix_with_bicluster: Matrix, base_config_params_for_detect: dict):
    config = replace(core_config_detection, clustering_method=ClusteringMethod.KMEANS, k1=3, k2=2)
...
def test_detect_simple_bicluster_spectral(core_config_detection: BiclusterConfig, simple_matrix_with_bicluster: Matrix, base_config_params_for_detect: dict):
    config = replace(core_config_detection,
...
def test_detect_n_clusters_param_effect(core_config_detection: BiclusterConfig, simple_matrix_two_biclusters: Matrix, base_config_params_for_detect: dict):
    config = replace(core_config_detection, k1=3, k2=2) # k1=iter, k2=default clusters_cols
    detector = SVDBiclusterDetector(config=config)
...
def test_detect_min_max_filters(core_config_detection: BiclusterConfig, base_config_params_for_detect: dict):
    matrix = np.array([
        [5,5,1,1], [5,5,1,1],
        [1,1,1,1], [2,2,2,2]  
    ])
    ...
    biclusters = detector.detect(matrix, 
                                 ...
                                 min_rows=2, 
                                 min_cols=2,
                                 max_overlap=1.0) # Override max_overlap
    ...
    for bc in biclusters:
        if np.array_equal(bc.row_indices, expected_row_indices) and \
           np.array_equal(bc.col_indices, expected_col_indices):
            found_expected_bicluster = True
            break
    assert found_expected_bicluster, "The specific 2x2 block [[5,5],[5,5]] was not found"

    # The second part of the test can remain as is, as it checks if allowing smaller biclusters finds more or equal.
    biclusters_min_1x1 = detector.detect(matrix, 
                                         n_iterations=config_strict.k1, 
                                         n_clusters_rows=config_strict.k2, 
                                         n_clusters_cols=config_strict.k2, 
                                         min_rows=1, 
                                         min_cols=1,
                                         max_overlap=1.0) # Override max_overlap here as well
    assert len(biclusters_min_1x1) >= len(biclusters)

def test_detect_no_bicluster_found(core_config_detection: BiclusterConfig, base_config_params_for_detect: dict):
    matrix = np.random.rand(10, 10) 
    config = replace(core_config_detection, k1=1, tolerance=0.001) # Very strict tolerance
    detector = SVDBiclusterDetector(config=config)
    biclusters = detector.detect(
        matrix, 
        n_iterations=config.k1, 
...
def test_svd_on_almost_zero_matrix(core_config_detection: BiclusterConfig, base_config_params_for_detect: dict):
    matrix = np.zeros((5,5)); matrix[0,0] = 1e-12
    config = replace(core_config_detection, k1=1, k2=1)
    detector = SVDBiclusterDetector(config=config)
    biclusters = detector.detect(
        matrix, 
        n_iterations=config.k1, n_clusters_rows=config.k2, n_clusters_cols=config.k2, n_svd_components=1,
        min_rows=base_config_params_for_detect["min_rows"], min_cols=base_config_params_for_detect["min_cols"]
    )
    assert isinstance(biclusters, list)

def test_max_overlap_filtering(core_config_detection: BiclusterConfig, base_config_params_for_detect: dict):
    config = replace(core_config_detection, k1=1, k2=3)
    matrix_overlap_prone = np.array([
        [10,10,10,1,1,1],[10,10,10,1,1,1],[10,10,10,1,1,1],
        [9,9,9,1.5,1.5,1.5],[1,1,1,10,10,10],[1,1,1,10,10,10],
    ])
    target_max_overlap = 0.2
    detector_overlap = SVDBiclusterDetector(config=config)
    biclusters = detector_overlap.detect(
        matrix_overlap_prone, n_iterations=config.k1, 
        n_clusters_rows=config.k2, n_clusters_cols=config.k2, 
        min_rows=base_config_params_for_detect["min_rows"],
        min_cols=base_config_params_for_detect["min_cols"],max_overlap=target_max_overlap
    )
    ...
    if len(biclusters) > 1:
        for i in range(len(biclusters)):
            for j in range(i + 1, len(biclusters)):
                ...
                assert max(j_row, j_col) <= target_max_overlap, \
